...
# Load Data
# --- Tabs Setup ---
tab_guide, tab_eda, tab_causal, tab_chat = st.tabs(["📘 User Guide", "📊 Exploratory Analysis", "🧪 Causal Analysis", "💬 AI Assistant"])
...
with tab_chat:
    ...
    if not chatbot_utils.configure_genai():
        st.warning("Google API Key not found. Please set `GOOGLE_API_KEY` in `.streamlit/secrets.toml` or environment variables to use the chatbot.")
        # Optional: Allow user to input key manually
        api_key_input = st.text_input("Or enter your Google API Key:", type="password")
        if api_key_input:
             import os
             import google.generativeai as genai
    ...
    # 4. Quick Start Questions (Only if last message is from assistant)
    ...
    # 5. Handle User Input (Text Box)
    if prompt := st.chat_input("Ask about your data or causal inference..."):
        st.session_state.messages.append({"role": "user", "content": prompt})
        st.rerun()

    # 6. Generate Response (Triggered if last message is user)
    if st.session_state.messages[-1]["role"] == "user":
