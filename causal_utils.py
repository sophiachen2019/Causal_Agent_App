...
"""
    ...
    if estimation_method == "Difference-in-Differences (DiD)":
        ...
        if use_logit:
            ...
            script += "print('Running Refutation...')\n"
            script += "# 1. Random Common Cause\n"
            script += "refute_results_rcc = model.refute_estimate(\n"
            script += "    identified_estimand,\n"
            script += "    estimate,\n"
            script += "    method_name='random_common_cause'\n"
            script += ")\n"
            script += "print(refute_results_rcc)\n"
            script += "\n"
            script += "# 2. Placebo Treatment\n"
            script += "refute_results_placebo = model.refute_estimate(\n"
            script += "    identified_estimand,\n"
            script += "    estimate,\n"
            script += "    method_name='placebo_treatment_refuter',\n"
            script += "    placebo_type='permute'\n"
            script += ")\n"
            script += "print(refute_results_placebo)\n"
    ...
    if hte_features:
        ...
        elif estimation_method in ["Linear Double Machine Learning (LinearDML)", "Meta-Learner: S-Learner", "Meta-Learner: T-Learner", "Generalized Random Forests (CausalForestDML)"]:
